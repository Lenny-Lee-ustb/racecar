...
private:
  ...
  ros::NodeHandle n_;
  ros::Subscriber odom_sub, path_sub, goal_sub,traj_sub;
  ...
  int car_stop;
  teb_local_planner::TrajectoryMsg odom_traj;

  void odomCB(const nav_msgs::Odometry::ConstPtr &odomMsg);
  void pathCB(const nav_msgs::Path::ConstPtr &pathMsg);
  void trajCB(const teb_local_planner::TrajectoryMsg& trajMsg);
...
L1Controller::get_odom_car2WayPtVec(const geometry_msgs::Pose &carPose) {
  ...
  {
    for (int i = 0; i < odom_traj.trajectory.size(); i++) {//map_path.poses.size()
	        geometry_msgs::Pose map_path_pose = odom_traj.trajectory[i].pose;
      //geometry_msgs::PoseStamped map_path_pose = map_path.poses[i]; //获取路径点
        geometry_msgs::PoseStamped odom_path_pose;
	      //geometry_msgs::Point odom_path_wayPt = map_path_pose.position;//frame_map=frame_odom
      try //可能抛出异常的语句
      {
        //tf_listener.transformPose("odom", ros::Time(0), map_path_pose, "map",
        //                          odom_path_pose);
        geometry_msgs::Point odom_path_wayPt =
            map_path_pose.position; //获得路径的位置点
...
void L1Controller::trajCB(const teb_local_planner::TrajectoryMsg& trajMsg)
{
    odom_traj = trajMsg;
}
...
L1Controller::L1Controller() {
  ...
  traj_sub = n_.subscribe("/move_base/TebLocalPlannerROS/traj_feedback", 1, &L1Controller::trajCB, this);
  goal_sub =
