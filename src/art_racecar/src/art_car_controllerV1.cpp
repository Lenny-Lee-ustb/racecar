...
double L1Controller::getYawFromPose(const geometry_msgs::Pose& carPose)
{
    ...
    float y = carPose.orientation.y;        //轴向量的x
    ...
    quaternion.getRPY(tmp,tmp, yaw);        //分别存放R/P/Y  R:横滚角 P:俯仰角  Y: 航向角  
    ...
    if(!goal_reached)                                   //未到达目标点
    {
        for(int i =0; i< map_path.poses.size(); i++)
        {
            ...
            try      //可能抛出异常的语句
            {
                ...
                geometry_msgs::Point odom_path_wayPt = odom_path_pose.pose.position;//获得路径的位置点
                bool _isForwardWayPt = isForwardWayPt(odom_path_wayPt,carPose);     //判断是否为前进点
                ...
                if(1||_isForwardWayPt)//是前进点
                {
                    ...
                    {
			forwardPtcontrolx=forwardPtcontrolwholex/(forwardPtcontrollinex+eps);
			forwardPtcontroly=forwardPtcontrolwholey/(forwardPtcontrolliney+eps);
			  ...
			  forwardPtcontrolwholex+=(odom_path_wayPt.x - carPose_pos.x)*(i+1)*(i+1);
			  forwardPtcontrolwholey+=(odom_path_wayPt.y - carPose_pos.y)*(i+1)*(i+1);
...
void L1Controller::controlLoopCB(const ros::TimerEvent&)
{
	...
	ROS_INFO("foundForwardPt = %s ",foundForwardPt?"True":"False");
	    ...
	    cmd_vel.angular.z = baseAngle + p_turn*turn_erro + d_turn*(turn_erro - last_turn_erro);
